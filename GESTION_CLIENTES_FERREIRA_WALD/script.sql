...
DELIMITER $$
CREATE PROCEDURE SP_UPD_NUMERADOR(P_EMPRESA INT, P_ENTIDAD VARCHAR(50))
BEGIN
	UPDATE NUMERADOR SET ULTIMO_NUM = ULTIMO_NUM + 1
...
BEGIN
    ...
    CALL SP_UPD_NUMERADOR(P_EMPRESA,'PRODUCTO');
END $$
DELIMITER ;
...
DELIMITER $$
CREATE PROCEDURE SP_INS_POLITICA(P_EMPRESA INT, P_NOMBRE VARCHAR(100), P_PRECIO DECIMAL(10,2))
BEGIN
	DECLARE P_ID_POLITICA INT;
    DECLARE P_ESTADO INT;
    SELECT FN_NEW_ID(P_EMPRESA,'POLITICA') INTO P_ID_POLITICA;
    SELECT FN_BUSCAR_ESTADO('POLITICA','ACTIVO') INTO P_ESTADO;
    INSERT INTO POLITICA (ID_EMPRESA,ID_POLITICA,POL_NOMBRE,POL_PRECIO,ID_ESTADO) VALUES (P_EMPRESA,P_ID_PRODUCTO,P_NOMBRE,P_ESTADO);
    CALL SP_UPD_NUMERADOR(P_EMPRESA,'POLITICA');
END $$
DELIMITER ;
...
DELIMITER $$
CREATE PROCEDURE SP_INS_CLIENTE(P_EMPRESA INT, P_NOMBRE VARCHAR(100), P_APELLIDO VARCHAR(100), P_DNI VARCHAR(30), P_EMAIL VARCHAR(100), P_TEL VARCHAR(20), P_DIR VARCHAR(100))
BEGIN
	DECLARE P_ID_CLIENTE INT;
    DECLARE P_ESTADO INT;
    SELECT FN_NEW_ID(P_EMPRESA,'CLIENTE') INTO P_ID_CLIENTE;
    SELECT FN_BUSCAR_ESTADO('CLIENTE','INGRESADO') INTO P_ESTADO;
...
FOR EACH ROW
BEGIN
	CALL SP_UPD_NUMERADOR(P_EMPRESA,'CLIENTE');
END $$
DELIMITER ;

DELIMITER $$
CREATE PROCEDURE SP_INS_CONTRATO(P_EMPRESA INT, P_PRODUCTO INT , P_POLITICA INT, P_CLIENTE INT, P_FCH_CONTRATO DATE, P_FCH_VIGENCIA DATE)
BEGIN
	DECLARE P_ID_CONTRATO INT;
    DECLARE P_ESTADO INT;
    SELECT FN_NEW_ID(P_EMPRESA,'CONTRATO') INTO P_ID_CONTRATO;
    SELECT FN_BUSCAR_ESTADO('CONTRATO','INGRESADO') INTO P_ESTADO;
    ...
    CALL SP_UPD_NUMERADOR(P_EMPRESA,'CONTRATO');
END $$
DELIMITER ;
